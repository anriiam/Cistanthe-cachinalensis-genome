...
def usage(message=None, exitcode=1, stream=sys.stderr):
    ...
    stream.write("   Break a multi-fasta file of scaffolds into a fasta file of contigs,\n")
    stream.write("   also writes a JuiceBox assembly file describing the changes.\n")
    stream.write("\n")
    stream.write("Usage:   %s [options] <in.fasta> <out-prefix>\n" % (__program__))
    stream.write("\n")
    stream.write("Options: -c,--break-on-center       Break gaps on their center bases\n")
    stream.write("         -D,--mv-Ns-to-debris       Move gap N sequences to debris pile\n")
    stream.write("         -g,--min-gap-length <int>  Min gap size to break contigs [1]\n")
    stream.write("         -G,--max-gap-length <int>  Max gap size to break contigs [-1]\n")
    stream.write("         -h,--help                  Print this help message and exit\n")
    stream.write("\n")
    stream.write("Notes:  Set --max-gap-length to a value < 0 to break every gap larger than\n")
    stream.write("        --min-gap-length\n\n")
    stream.write("%s\n" % message)
    sys.exit(exitcode)
